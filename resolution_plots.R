...
library(parallel)
source(paste(pathToFile,"/scripts_resolution/breakpoint_functions.R", sep = ""))

## setting up chrom
chrNb <- as.numeric(args[1])
#chrNb <- 14

print(paste0("Plotting recombination resolution for chrm: ", chrNb))
chrTag <- paste0("chr", chrNb) 

#ADD commands defining the data frame file name
# open the data frame and save is as df
setwd(paste0(pathToFile, "/", chrTag, "/resolution/resolution.95/"))
...
cat(paste0("Ind", "\t", "MeanNbOfBps", "\t", "NbOfCells", "\t", "MedBpSize", "\n"), 
    file=paste0(pathToFile, "/", chrTag, "/resolution/resolution.95/summaryBp_", chrTag, ".txt"), sep = "")
...
for (i in 1:length(dataframes)) {
  ...
  png(file = paste0(pathToFile, "/", chrTag, "/resolution/resolution.95/", indID, "_", chrTag, "_resolutionWGS.95.png"), height = 300, width = 800)
  ...
  dev.off()
  cat(paste0(indID, "\t", round(sum(nb_of_breakpoints[[i]][-c(which(nb_of_breakpoints[[i]] > quantile(nb_of_breakpoints[[i]], probs = q.lim)))])/nb_of_cells[[i]], digits = 2),
             "\t", nb_of_cells[[i]], "\t", round(medSizeBreakpoint_clean[[i]], digits = 2), "\n"), 
      file=paste0(pathToFile, "/", chrTag, "/resolution/resolution.95/summaryBp_", chrTag, ".txt"), sep = "", append = TRUE)
}
