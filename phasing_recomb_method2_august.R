...
##
## MAIN FUNCTION
## this script can be run as: qsub -P awadallalab -V -cwd -b y -pe smp 5 -l h_vmem=8G ./launching_sc_method2.bash S84 chr21
## using the bash script indicated as hosted in my HOME
##########################################
##########################################
...
wrkDir <- "/.mounts/labs/awadallalab/scratch/ialves/sc_recombination"
inputDir <- "/.mounts/labs/awadallalab/scratch/SingleCell/vcfs_sc/allSamples"
chrToFetch <- args[2]
indToFetch <- args[1]

#input
files <- list.files(path=paste0(inputDir, "/", indToFetch), pattern=paste0("*", chrToFetch,".GT.FORMAT"))
file.copy(from = paste0(inputDir, "/", indToFetch, "/", files), to = wrkDir, copy.mode = T, overwrite = T )

setwd(wrkDir)
openGT <- read.table(files, header = T)
#be careful that this might or might not be the indTAG so please do not change it if you think it is redundant
...
#output
output_prefix <- sub(".GT.FORMAT", "", files)
...
#remove cells with <50 variants covered
...
#removing variants with covered by <3*sd(mean nb of cells covering a variant)
...
while(targetCell <= (ncol(openGT)-2)) {
  ...
  while(comparingCell <= (ncol(tmpOpenGT)-2)) {
   ...
   #   }
   # }
   mu <- mean(similarity, na.rm = T)
   var <- var(similarity, na.rm = T)
   estBetaParams <- function(mu, var) {
      alpha <- ((1 - mu) / var - 1 / mu) * mu ^ 2
      beta <- alpha * (1 / mu - 1)
      return(params = list(alpha = alpha, beta = beta))
      }
   paramBeta <- estBetaParams(mu, var)
	    
   #test whether mu and var are positive
    if (paramBeta$alpha > 0 & paramBeta$beta > 0) { #added Aug 18 2020 - to take into account cells for with alpha and beta values lower than 0.

    testDist <- ks.test(similarity, rbeta(length(similarity[!is.na(similarity)]), shape1 = paramBeta$alpha, shape2 = paramBeta$beta), )
    print(paste0("Alpha=",paramBeta$alpha, "and beta=",paramBeta$beta))
			      
    #get the maximum ratio alpha/beta
    ratio=seq(0.20,0.90, length.out = 8)
    alphaBoundary=rev(seq(0.05, 0.75, length.out = 8))
    rThreshold=ratio[max(which(paramBeta$alpha < alphaBoundary))]
		              
    #  
    if(paramBeta$alpha < 0.75 & paramBeta$beta < 0.75 & abs(1-(paramBeta$alpha/paramBeta$beta)) <= rThreshold) {
          print(paste0("p-value: ", testDist$p.value))
          cat(nameCell, file = paste0(wrkDir, "/", indTAG, "/", chrID, "/", output_prefix, ".", nameCell,".parentalCells.txt"), sep = "\n")
          if(length(which(similarity < 0.05 | similarity > 0.95)) > 0) {
              for(cell in which(similarity < 0.05 | similarity > 0.95)) {
                  tmp_cellName <- unlist(strsplit(colnames(tmpOpenGT)[cell+3], split = "\\."))[2]
...
##################################
##
## This part will take the cells identified as representing 
...
if(length(parentalCells) > 10) { #the remaining code will be run ONLY and ONLY if there are at least 10 parental cells.
  ...
  for(file in parentalCells) {
    ...
    #file <- parentalCells[COUNT_CELLS]
    readFile <- read.table(paste0(pathToIndDir, "/", file), skip = 1)
    interCells <- intersect(as.character(readFile$V1), colnames(similarity_m))
    if(length(interCells) > 0) { #added Aug 18 2020 to take into account cells with no overlapping info with other parentalCells
      similarity_m[COUNT_CELLS,match(interCells, colnames(similarity_m))] <- readFile$V2[match(interCells,readFile$V1)]
    } else {
      excludeCells <- c(excludeCells,COUNT_CELLS)
    }
    COUNT_CELLS <- COUNT_CELLS+1
  }
  if(length(excludeCells) > 0) { #added Aug 18 2020 to take into account cells with no overlapping info with other parentalCells
    #this means that an initially considered parental cell does not have information on the similarity with other inferred parental cells
    # therefore it needs to be removed from the simmetrical similarity matrix used to compute parental haplotypes. 
    file.remove(paste0(wrkDir, "/", indTAG, "/", chrID, "/", output_prefix, ".", colnames(similarity_m)[excludeCells],".parentalCells.txt"))
    similarity_m <- similarity_m[-excludeCells, -excludeCells]
    parentalCells <- parentalCells[-excludeCells]
    indxParentCells <- indxParentCells[-excludeCells]
  }
  ...
  #newOpenGT <- newOpenGT[,match(parentalONE, listParentCells)+2]
  ...
  while (cellNb <= ncol(subSet_SC)) {
    ...
    if(COUNT_CELLs == 1) {
      ...
      outfileName <- paste0(pathToIndDir, "/SC_phasedHapHMap_", indTAG,"_",chrID, "_QCvar25_NbCells10_NbLinks5_", COUNT_PDF,".pdf") 
      #S23_chr9_QCvar25_NbCells10_NbLinks5.pdf
      pdf(file=outfileName, height = 11, width = 6)
      par(mfrow=c(5,1))
      
    } else if (COUNT_CELLs == 6) {
      dev.off()
      COUNT_CELLs <- 1
    ...
    if(length(index_to_compare) >= 10) {
      ...
      plot(hapOne, type="l", col="blue", ylim=c(0,1), main = paste0(colnames(subSet_SC)[cellNb]))
      lines(hapTwo, type="l", col="red")
      # windowsAcrossCells[[cellNb]] <- listWindows
      # propAcrossCells[[cellNb]] <- rbind(hapOne,hapTwo) 
    } else {
      cells_too_few_sites <- c(cells_too_few_sites,colnames(subSet_SC)[cellNb])
    }
    cellNb <- cellNb+1
    COUNT_CELLs <- COUNT_CELLs+1
  }
  if(cellNb > ncol(subSet_SC) & COUNT_CELLs <= 6){
    dev.off()
  }
  #writing list of cells with too few sites < 10
  if(length(cells_too_few_sites) > 0) {
    write.table(matrix(cells_too_few_sites, ncol = 1),file=paste0(pathToIndDir, "/tooFewSites_", indTAG,"_",chrID, "_afterPhasing.txt"), quote = F,
  ...
  errorRates <- read.table(paste0(pathToIndDir, "/", indTAG, ".SingleCellsHetSNPs.",chrID,".errorRatePerSNP.txt"), header = F, skip = 1)
  nrow(errorRates)
  ...
  startProb <- 0.5
  dim(openGT)
  #removing cells with very few sites 
  if(length(cells_too_few_sites) > 0) {
    openGT <- openGT[,setdiff(colnames(openGT),cells_too_few_sites)]
    dim(openGT)
  }
  ...
  hmm_results <- mclapply(1:(ncol(openGT)-2), function(x) { inferring_bp_hmm(x) }, mc.cores = no_cores)
  ...
  subVarNames <- as.numeric(rownames(cell_hmm_seq_m))
  ...
  while (cellNb <= ncol(subSet_SC)) {
    ...
    if(COUNT_CELLs == 1) {
      ...
      outfileName <- paste0(pathToIndDir, "/SC_phasedHapHMap_", indTAG,"_",chrID, "_QCvar25_NbCells10_NbLinks5_", COUNT_PDF,"_postHMM.pdf") 
      #S23_chr9_QCvar25_NbCells10_NbLinks5.pdf
      pdf(file=outfileName, height = 11, width = 6)
      par(mfrow=c(5,1))
  ...
  if(sum(lengths(chunksLengths) > 8) > 0) {
    ...
    list_hmm_clean <- cell_hmm_StateSeq_m
    cellNames_clean <- cellNames_v
    chunksLengths_clean <- chunksLengths
    stateSequences_clean <- stateSequences
    coordinatesStart_clean <- coordinates_start
    coordinatesEnd_clean <- coordinates_end
